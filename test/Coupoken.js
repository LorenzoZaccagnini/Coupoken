...
contract("Coupoken", accounts => {
  ...
  it('should create a merchant', async() => {
      ...
      assert.equal(merchantInfo[1], 'Bob');
      assert.equal(merchantInfo[2], 'Clothes');
      assert.equal(merchantInfo[3], 'test.com');
  });
  ...
  it('should not duplicate a merchant address', async() => {
      ...
      await truffleAssert.reverts(
          coupoken.createMerchant('Alice', 'Clothes', 'test2.com', {from: user1}),
  ...
  it('should create a coupon', async() => {
      const coupoken = await Coupoken.deployed()
      let user1 = accounts[1]
      await coupoken.createCoupon('50', '1000', '999999999', '1', 'test.com', {from: user1})
      ...
      assert.equal(couponInfo[6], user1);
  });
  ...
  it('should buy the coupon', async() => {
    ...
    await coupoken.createCoupon('50', couponPrice, '999999999', cp1, 'test.com', {from: user1})
    ...
    console.log("Balance of user after buy: ", await web3.eth.getBalance(user2));
