...
const ListAssets = props => {
  ...
  const toggleLock = async () => {
    ...
    const stackId = contract.methods["toggleLockCoupon"].cacheSend(!item.tradable, {
  ...
  const getCoupons = async () => {
      const contract = await drizzle.contracts.Coupoken
      let coupon = await contract.methods.getCouponInfo(id)
        .call({from: drizzleState.accounts[0]})
      coupon.owner = await contract.methods.ownerOf(id)
          .call({from: drizzleState.accounts[0]})
      let merchant = await contract.methods.getMerchantInfo(coupon.merchantAdr)
        .call({from: drizzleState.accounts[0]})
      let uri = await contract.methods.tokenURI(id)
        .call({from: drizzleState.accounts[0]})
                                    ...
                                    </div>
                                    {
                                      ...
                                      <div className="">
                                        <form onSubmit={handleSubmit(setPriceCoupon)}>
                                          ...
                                          <input className="button-primary" type="submit" value="Change Price" />
                                        </form>
                                          ...
                                          {
                                            item.tradable ?
                                            <input class="button-primary" type="button" value="Lock Asset" onClick={() => toggleLock()} />
                                            :
                                            <input class="button-primary" type="button" value="Unlock Asset" onClick={() => toggleLock()} />
                                          }
                                          <div>{getTxStatus()}</div>
                                      </div>
